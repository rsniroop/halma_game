...
class halma_ai_agent:
    ...
    def get_nextMove(self):
        ...
        num_moves = self.game.moves_completed
        if num_moves != -1 and num_moves < 7 :
    ...
    def __init__(self, game_type, player, play_time, board):
        ...
        if self.game_type == "SINGLE":
            ...
            if os.path.exists("playdata.txt"):
                ...
                with open("playdata.txt", "w+") as playdata_file:
                    num_moves = 0
    ...
    def dump_move(self):
        ...
        if self.game_type == "GAME":
            ...
            with open("playdata.txt", "w+") as data_file:
                ...
                key1 = ','.join(map(str, suggested_move[0]))
                self.json_data['moves'][','.join(map(str, self.suggested_move[0]))] = self.suggested_move[1]
                if key1 in self.json_data['destined_pos']:
                    self.json_data['destined_pos'][','.join(map(str, suggested_move[1]))] = self.json_data['destined_pos'][key1]
                    del self.json_data['destined_pos'][key1]
